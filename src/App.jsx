...
// ─── MEMORY LAYER ─────────────────────────────────────────────────────────────
// All localStorage lives here. Swap these out later for a real DB (Supabase etc.)
// The rest of the app never touches localStorage directly — only these functions do.

const STORAGE_KEYS = {
  APP: 'lockapp',
  ODDS_CACHE: 'oddsCache',
  PROPS_CACHE: 'propsCache',     // Tank01 player props (legacy)
  PROPS_DAY_CACHE: 'propsDayCache', // the-odds-api props: { [dateKey_eventId]: { lines, fetchedAt } }
  STATS_CACHE: 'statsCache',     // balldontlie player stats
...
// ─── MEMORY LAYER ─────────────────────────────────────────────────────────────
// All data goes through server.js → savedata.json instead of localStorage.
// Cache data (odds, props, stats) stays in localStorage — it's temporary anyway.
...
async function saveState(state) {
  try {
    ...
    await fetch(`${SERVER}/data`, {
      ...
      body: JSON.stringify({ ...await loadAllData(), app: state })
    })
...
// Odds cache stays in localStorage — temporary data, no need to persist
...
async function savePredictions(pred) {
  try {
...
async function saveLayHistory(lay) {
  try {
    const current = await loadAllData()
...
function calcProfit(odds, stake = 1) {
...
function calcPayout(odds, stake = 1) {
...
function combineParlayOdds(legOdds) {
  if (!legOdds.length) return null
  // Convert each leg to decimal, multiply, convert back
